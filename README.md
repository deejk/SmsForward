...
- 📊 **월 누적 요약**: 카드별(삼성/신한) 당월 누적 사용액을 전달 문자 하단에 첨부
...
- **누적 요약은 추정치**: 앱이 받은 승인 건만 달력 월 단위로 자체 합산 — 서비스 중단 중 결제·결제 취소는 미반영. 카드사 공식 청구액과 다를 수 있음 (SMS 내 `누적` 표기는 월 합계가 아니라 미사용)
...
| `삼성1745승인 ... 17,000원` 연속 수신 | 전달 문자 하단에 `📊 N월 누적 사용 / 삼성: 합산액` 표시 ✅ |
| 폰 재부팅 | 서비스 자동 재시작 ✅ |
